/// <summary>
/// Auto generated offsets for PUBG 
/// Extracted from https://github.com/TJ888/PUBG-SDK
/// </summary>

namespace Offsets
{
	public class UPopupWidgetForReplay_C
	{
		///<summary>FPointerToUberGraphFrame</summary>
		public const int UberGraphFrame = 0x0240;

		///<summary>UWidgetAnimation *</summary>
		public const int PopupEmerging = 0x0248;

		///<summary>UButton *</summary>
		public const int ButtonCancel = 0x0250;

		///<summary>UButton *</summary>
		public const int ButtonOK = 0x0258;

		///<summary>UCircularThrobber *</summary>
		public const int CircularThrobber_1 = 0x0260;

		///<summary>UTextBlock *</summary>
		public const int TextBlock_172 = 0x0268;

		///<summary>UTextBlock *</summary>
		public const int TextCancel = 0x0270;

		///<summary>UTextBlock *</summary>
		public const int TextOK = 0x0278;

		///<summary>UBorder *</summary>
		public const int Waiting = 0x0280;

		///<summary>FText</summary>
		public const int Message = 0x0288;

		///<summary>FScriptMulticastDelegate</summary>
		public const int ButtonClickDispatcher = 0x02A0;

		///<summary>FScriptMulticastDelegate</summary>
		public const int HideMyself = 0x02B0;

	}

}
