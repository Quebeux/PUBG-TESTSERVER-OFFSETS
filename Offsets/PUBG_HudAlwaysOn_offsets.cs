/// <summary>
/// Auto generated offsets for PUBG 
/// Extracted from https://github.com/TJ888/PUBG-SDK
/// </summary>

namespace Offsets
{
	public class UHudAlwaysOn_C
	{
		///<summary>FPointerToUberGraphFrame</summary>
		public const int UberGraphFrame = 0x0240;

		///<summary>UBoostGaugeWidget_BP_C *</summary>
		public const int BoostGaugeWidget_BP = 0x0248;

		///<summary>UBorder *</summary>
		public const int Border_1 = 0x0250;

		///<summary>UBP_AliveHumansInfoWidget_C *</summary>
		public const int BP_AliveHumansInfoWidget_145 = 0x0258;

		///<summary>UBP_AlivePlayerInfoWidget_C *</summary>
		public const int BP_AlivePlayerInfoWidget = 0x0260;

		///<summary>UBP_AliveZombiesInfoWidget_C *</summary>
		public const int BP_AliveZombiesInfoWidget_98 = 0x0268;

		///<summary>UBP_BuffIconListWidget_C *</summary>
		public const int BP_BuffIconListWidget = 0x0270;

		///<summary>UBP_LifeGaugeTemplateWidget_C *</summary>
		public const int BP_LifeGaugeTemplateWidget = 0x0278;

		///<summary>UBP_StanceManagerWidget_C *</summary>
		public const int BP_StanceManagerWidget = 0x0280;

		///<summary>UBP_TeamInfoListWidget_C *</summary>
		public const int BP_TeamInfoListWidget = 0x0288;

		///<summary>UCastingBarInfoTextWidget_C *</summary>
		public const int CastingBarInfoTextWidget = 0x0290;

		///<summary>UCanvasPanel *</summary>
		public const int CharacterCanvas = 0x0298;

		///<summary>UKillCountWidget_C *</summary>
		public const int KillCountWidget = 0x02A0;

		///<summary>UNetworkProblemWidget_C *</summary>
		public const int NetworkProblemWidget = 0x02A8;

		///<summary>UObserverTeamInfoList_C *</summary>
		public const int ObserverTeamInfoList = 0x02B0;

		///<summary>UParachuteVehicleWidget_C *</summary>
		public const int ParachuteVehicleWidget_862 = 0x02B8;

		///<summary>UReplayProblemWidget_C *</summary>
		public const int ReplayProblemWidget = 0x02C0;

		///<summary>UButton *</summary>
		public const int ReportBotton = 0x02C8;

		///<summary>URetainerBox *</summary>
		public const int RetainerBox_VersionInfo = 0x02D0;

		///<summary>UTeamCountWidget_C *</summary>
		public const int TeamCountWidget = 0x02D8;

		///<summary>UVersionInfoWidget_C *</summary>
		public const int VersionInfoWidget_96 = 0x02E0;

		///<summary>UWidgetSwitcher *</summary>
		public const int WidgetSwitcher_1 = 0x02E8;

		///<summary>ATslHUD *</summary>
		public const int refHUD = 0x02F0;

		///<summary>ATslCharacter *</summary>
		public const int CachedCharacter = 0x02F8;

		///<summary>FWidgetTransform</summary>
		public const int CharacterInfoDownPos = 0x0300;

		///<summary>FWidgetTransform</summary>
		public const int CharacterInfoUpPos = 0x031C;

	}

}
