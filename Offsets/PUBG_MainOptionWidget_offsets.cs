/// <summary>
/// Auto generated offsets for PUBG 
/// Extracted from https://github.com/TJ888/PUBG-SDK
/// </summary>

namespace Offsets
{
	public class UMainOptionWidget_C
	{
		///<summary><![CDATA[FPointerToUberGraphFrame]]></summary>
		public const int UberGraphFrame = 0x0290;

		///<summary><![CDATA[UOptionSettingsButotn_C *]]></summary>
		public const int ApplyButton = 0x0298;

		///<summary><![CDATA[UOptionSettingsButotn_C *]]></summary>
		public const int CancelButton = 0x02A0;

		///<summary><![CDATA[UOptionSettingsButotn_C *]]></summary>
		public const int DefaultButton = 0x02A8;

		///<summary><![CDATA[UGameplayOptionWidget_BP_C *]]></summary>
		public const int GameOptionWidget_BP = 0x02B0;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_1 = 0x02B8;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_2 = 0x02C0;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_6 = 0x02C8;

		///<summary><![CDATA[UKeyOptionWidget_BP_C *]]></summary>
		public const int KeyOptionWidget_BP = 0x02D0;

		///<summary><![CDATA[UOptionSettingsButotn_C *]]></summary>
		public const int OkButton = 0x02D8;

		///<summary><![CDATA[UOptionButton_C *]]></summary>
		public const int OptionButton0 = 0x02E0;

		///<summary><![CDATA[UOptionButton_C *]]></summary>
		public const int OptionButton1 = 0x02E8;

		///<summary><![CDATA[UOptionButton_C *]]></summary>
		public const int OptionButton2 = 0x02F0;

		///<summary><![CDATA[UOptionButton_C *]]></summary>
		public const int OptionButton3 = 0x02F8;

		///<summary><![CDATA[UHorizontalBox *]]></summary>
		public const int OptionButtonHorizontalBox = 0x0300;

		///<summary><![CDATA[UWidgetSwitcher *]]></summary>
		public const int OptionSwitch = 0x0308;

		///<summary><![CDATA[UOptionTitleWidget_C *]]></summary>
		public const int OptionTitleWidget = 0x0310;

		///<summary><![CDATA[UOptionSettingsButotn_C *]]></summary>
		public const int ResetButton = 0x0318;

		///<summary><![CDATA[USoundOptionWidget_C *]]></summary>
		public const int SoundOptionWidget = 0x0320;

		///<summary><![CDATA[UGraphicOptionWidget_C *]]></summary>
		public const int VedioOptionWidget = 0x0328;

		///<summary><![CDATA[UTslBaseOptionWidget *]]></summary>
		public const int SelectedOption = 0x0330;

		///<summary><![CDATA[FText]]></summary>
		public const int DisplayOptoinText = 0x0338;

	}

}
